...
This packages contains functions that allow you to wrap existing ESLint rules, plugins, and configurations that were intended for use with ESLint v9.x to allow them to work as-is in ESLint v10.x.

**Note:** All plugins are not guaranteed to work in ESLint v10.x. This package fixes the most common issues but can't fix everything.
...
If you have a rule that you'd like to make compatible with ESLint v10.x, you can do so using the `fixupRule()` function:
...
If you are using a plugin in your `eslint.config.js` that is not yet compatible with ESLint 10.x, you can wrap it using the `fixupPluginRules()` function:
...
If you are importing other configs into your `eslint.config.js` that use plugins that are not yet compatible with ESLint 10.x, you can wrap the entire array or a single object using the `fixupConfigRules()` function:
